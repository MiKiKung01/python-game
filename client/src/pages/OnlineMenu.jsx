...
export default function OnlineMenu() {
    ...
    const handleCreateRoom = async (e) => {
        ...
        try { const res = await axios.post('http://localhost:3001/rooms/create', { ...createData, hostId: user.id }); navigate(`/lobby/${res.data.roomId}`); } catch { alert("เกิดข้อผิดพลาดในการสร้างห้อง"); }
    };
    ...
    const handleSaveSettings = async () => {
        try {
            ...
            alert("บันทึกการตั้งค่าเรียบร้อย!"); setShowSettingsModal(false);
        } catch { alert("ไม่สามารถเปลี่ยนชื่อได้"); }
    };

    const menuItems = [
        { name: "สุ่มห้อง", sub: "Quick Join", icon: <Shuffle size={28} />, color: "from-purple-500 to-purple-700", action: () => navigate('/matchmaking') },
        { name: "สร้างห้อง", sub: "Create", icon: <PlusSquare size={28} />, color: "from-green-500 to-green-700", action: () => setShowCreateModal(true) },
        { name: "ค้นหาห้อง", sub: "Join List", icon: <Users size={28} />, color: "from-blue-500 to-blue-700", action: () => navigate('/join-room') },
        { name: "เล่นคนเดียว", sub: "Solo", icon: <User size={28} />, color: "from-cat-orange to-cat-amber", action: () => navigate('/menu') },
        { name: "ตั้งค่า", sub: "Settings", icon: <Settings size={28} />, color: "from-slate-500 to-slate-700", action: () => setShowSettingsModal(true) },
        { name: "ย้อนกลับ", sub: "Back", icon: <LogOut size={28} />, color: "from-red-500 to-red-700", action: () => navigate('/menu') },
    ];
        ...
        <div className="min-h-screen relative transition-colors duration-300 overflow-y-auto">
            ...
            <div className="z-10 w-full max-w-4xl">
                <h1 className={`text-4xl md:text-5xl font-black text-center mb-12 transition-all duration-1000 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-8'}`}>
                    <span className="bg-gradient-to-r from-t-accent via-blue-400 to-t-accent-2 bg-clip-text text-transparent">ONLINE LOBBY</span>
                </h1>
            ...
            {showCreateModal && (
                <div className="fixed inset-0 bg-t-overlay backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in">
                    <div className="glass-panel rounded-2xl p-6 w-full max-w-md relative animate-scale-in border border-t-border">
                        ...
                        <h2 className="text-xl font-black text-t-text mb-6 flex items-center gap-2"><PlusSquare size={20} className="text-t-success" /> CREATE ROOM</h2>
                        <form onSubmit={handleCreateRoom} className="space-y-4">
                            <div>
                                <label className="text-t-accent text-xs font-bold tracking-widest">ROOM NAME</label>
                            ...
                            <div>
                                <label className="text-t-accent text-xs font-bold tracking-widest">MAX PLAYERS ({createData.maxPlayers})</label>
                            ...
                            <div>
                                <label className="text-t-accent text-xs font-bold tracking-widest">PASSWORD (Optional)</label>
                                <input type="text" placeholder="Leave empty for public"
                            ...
                            <button type="submit" className="w-full bg-t-success hover:bg-t-success/80 text-white font-bold py-3 rounded-xl transition-all duration-300 hover:shadow-[0_0_20px_var(--t-glow)] active:scale-[0.98] mt-2">CREATE & JOIN</button>
                        </form>
                    </div>
                </div>
            )}
            ...
            {showSettingsModal && (
                <div className="fixed inset-0 bg-t-overlay backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in">
                    <div className="glass-panel rounded-2xl p-6 w-full max-w-md relative animate-scale-in border border-t-border">
                        ...
                        <h2 className="text-xl font-black text-t-text mb-6 flex items-center gap-2"><Settings size={20} className="text-t-accent" /> SETTINGS</h2>
                        <div className="space-y-6">
                            <div>
                                <label className="text-cat-amber text-xs font-bold flex items-center gap-2 tracking-widest"><Volume2 size={14} /> MUSIC VOLUME</label>
                            ...
                            <div>
                                <label className="text-t-accent text-xs font-bold flex items-center gap-2 tracking-widest"><User size={14} /> DISPLAY NAME</label>
                                <input type="text" className="w-full bg-t-input border border-t-border text-t-text p-2.5 rounded-xl mt-1.5 focus:outline-none focus:border-t-border-accent focus:shadow-[0_0_15px_var(--t-glow)] transition-all"
                                    value={settingData.newName} onChange={(e) => setSettingData({ ...settingData, newName: e.target.value })} />
                            </div>
                            <button onClick={handleSaveSettings} className="w-full bg-t-accent hover:bg-t-accent-hover text-white font-bold py-3 rounded-xl transition-all duration-300 hover:shadow-[0_0_20px_var(--t-glow)] active:scale-[0.98] flex items-center justify-center gap-2"><Save size={16} /> SAVE CHANGES</button>
                        </div>
                    </div>
                </div>
            )}
